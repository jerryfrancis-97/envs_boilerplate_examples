...
class Data_recorder:
    ...
    def save_recordings(self, dumping_object, dump_path):
        fp = open(dump_path, 'wb')
        pk.dump(dumping_object, fp)
        fp.close()
        print(f"dumping_object dumped in {dump_path}!")
    
    def extract_recordings(self, extracting_path):
        fp = open(extracting_path, "rb")
        new_obj = pk.load(fp)
        print("Recording extracted!")
        return new_obj
...
class TRPOAgent:
    ...
    def train(self, env_name, logger, seed=None, batch_size=12000, iterations=100,
        ...
        for iteration in range(iterations):
            ...
            recording['average_reward_over_iteration'].append(0)
